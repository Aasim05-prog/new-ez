...
const BrowseNotes = () => {
  ...
  // Auto-set education level for logged-in users on first load
  useEffect(() => {
    if (user?.educationLevel && selectedLevel === 'All') {
      setSelectedLevel(user.educationLevel);
    }
  ...
  const filterContent = (
    <>
      ...
      {/* Clear */}
      <button className="btn btn-outline btn-sm w-full" onClick={() => { clearFilters(); setFiltersOpen(false); }}>
        Clear All Filters
      </button>
    </>
  );
    ...
    <div className="container min-h-screen" style={{ paddingTop: '10vh', paddingBottom: 'var(--space-20)' }}>
      <div className="text-center my-12 animate-fade-in-up">
        ...
        <p className="text-muted text-lg max-w-2xl mx-auto">
          {user?.educationLevel
            ? `Showing results for ${user.educationLevel}. Change the filter to explore other levels.`
            : "Find study materials from Std 1 to Master's degree and government exam preparation."}
        </p>
        <div className="flex justify-center gap-3 mt-6" style={{ flexWrap: 'wrap' }}>
          <button onClick={() => { setShowTextbooks(false); setSelectedLevel(user?.educationLevel || 'All'); setSelectedSubject('All'); }}
      ...
      <button className="browse-filter-toggle mb-6" onClick={() => setFiltersOpen(true)}>
        🔍 Filters {selectedLevel !== 'All' || selectedSubject !== 'All' ? `(${[selectedLevel !== 'All' ? selectedLevel : '', selectedSubject !== 'All' ? selectedSubject : ''].filter(Boolean).join(', ')})` : ''}
      </button>

      <div className="flex flex-wrap gap-8" style={{ alignItems: 'flex-start' }}>
        
        {/* Desktop Sidebar Filters */}
        <aside className="card flex-shrink-0 animate-fade-in hide-mobile" style={{ width: '100%', maxWidth: '280px', padding: 'var(--space-6)', position: 'sticky', top: '100px' }}>
        ...
        <div
          className={`browse-sidebar ${filtersOpen ? 'open' : ''}`}
          onClick={(e) => { if (e.target === e.currentTarget) setFiltersOpen(false); }}
        >
          <div className="browse-sidebar-inner card" style={{ padding: 'var(--space-6)' }}>
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-xl">Filters</h3>
              <button onClick={() => setFiltersOpen(false)} style={{ background: 'none', border: 'none', fontSize: '1.25rem', cursor: 'pointer', color: 'var(--text-muted)' }}>✕</button>
            </div>
            {filterContent}
            <button className="btn btn-primary w-full mt-4" onClick={() => setFiltersOpen(false)}>
              Apply Filters
            </button>
          </div>
        </div>
        ...
        <main style={{ flex: 1, minWidth: '0' }}>
          ...
          {loading ? (
            ...
            <>
              ...
              {/* Pagination */}
              {totalPages > 1 && (
                <div className="flex justify-center items-center gap-4 mt-12" style={{ flexWrap: 'wrap' }}>
            ...
            <div className="card text-center py-16 animate-fade-in-up">
              ...
              <button className="btn btn-outline mt-6" onClick={clearFilters}>
                Clear All Filters
              </button>
            </div>
          )}
